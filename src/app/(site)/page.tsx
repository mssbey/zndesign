...
import { HomeSlider, FeaturedProducts } from "@/components/woodmart-home";
...
export default async function Home() {
  ...
  return <>
    ...
    <section className="wrap wd-section" id="kategoriler"><div className="wd-section-heading"><p>ZN DESIGN KOLEKSİYONLARI</p><h2>ÖNE ÇIKAN KATEGORİLER</h2><p>Yaşam alanınız için tasarlanan konforlu ve zamansız seçenekler.</p></div><div className="wd-category-grid">{categories.filter(c => c.slug !== "bazalar").map((c, i) => <Link className={i === 0 ? "wd-category-tile large" : "wd-category-tile"} href={`/urunler?kategori=${c.slug}`} key={c.slug}><Photo src={c.image} alt={c.name}/><div><h3>{c.name}</h3><p>{products.filter(p => p.category === c.slug).length} ürün</p></div></Link>)}</div></section>
    ...
    <section className="wd-service-strip wrap"><div><span>◇</span><h3>Özenli tasarım</h3><p>Her detayda konfor</p></div><div><span>▱</span><h3>Zengin koleksiyon</h3><p>Yaşamınıza uygun seçenekler</p></div><div><span>✂</span><h3>Özel üretim</h3><p>Size özel ölçü ve kumaşlar</p></div><div><span>⌂</span><h3>Mağazamızı ziyaret edin</h3><Link href="/magazamiz">Esenyurt / İstanbul →</Link></div></section>
  </>;
}
